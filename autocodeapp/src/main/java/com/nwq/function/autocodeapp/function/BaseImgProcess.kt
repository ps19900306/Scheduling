...
class BaseImgProcess(
    ...
    fun getDatumPoint(datumPointSize: Int = 1): List<FeatureCoordinatePoint>? {
        var list =
            colorMaps[differenceKey]?.filter { it.isIdentificationKey && it.mDirectorPoint == null && it.mDirectorPointKey == null }
        if (list.isNullOrEmpty()) {
            list =
                colorMaps[brightestKey]?.filter { it.isIdentificationKey && it.mDirectorPoint == null && it.mDirectorPointKey == null }
        }
        if (list.isNullOrEmpty()) {
